function [X, X_mh, X_lfoot, X_rfoot, X_com, X_zmp, tau,W] = step(state_current1, state_final1, sf, T, T_swingi, T_swingf,f,no)
%l_foot=0.1;
...
% figure;
% hold all;
% plot(0:1/50:1, X1(1,:),'*','color','r');
% 
% plot(0:1/50:1, X2(1,:),'o','color','g');
% 
% plot(0:1/50:1, X3(1,:),'color','b');
% scatter([0 (ta-1)/50 (tb-1)/50 1],[X2(1,1) X2(1,ta) X2(1,tb) X2(1,end)],'filled','MarkerEdgeColor',[0 .2 .2],...
%     'MarkerFaceColor',[0 .1 .1],...
%     'LineWidth',1.5);
% scatter([0  (ta-1)/50 (tb-1)/50 1],[X1(1,1) X1(1,ta) X1(1,tb) X1(1,end)],'filled','MarkerEdgeColor',[0 .2 .2],...
%     'MarkerFaceColor',[0 .1 .1],...
%     'LineWidth',1.5);
% xlabel('time (s)');
% ylabel('x direction (m)');
% legend('Swing Foot','Mid-Hip','Stance Foot');
%%
...
for i=1:n
    ...
    %     if i>2 && norm(p_com(:,i)-p_com(:,i-1))>0.2
    %         disp(o2(:,:,i)),disp(i);
    %     end
    %%%%%%%%ZMP TEST %%%%%%%%%%%%%%%%%
    if sf==0
...
% 
% if no==1
%     plot(0:1/50:1,plott,'color','k','linewidth',1);
% elseif no==2
%     plot(0:1/50:1,plott,'color','b','linewidth',1);
% elseif no==3
%     plot(0:1/50:1,plott,'color','g','linewidth',1);
% elseif no==4
%     plot(0:1/50:1,plott,'color','r','linewidth',1);
%end

%save('gaJA2.mat','theta'); %for step length of 0.1m on level ground
...
for i=1:n
    ...
    Xzmp = pos_zmp(1,i);
    Yzmp = pos_zmp(2,i);
    if i>td1+1 && i<td2-1  && i~=36 && inpolygon(Xzmp,Yzmp,Xp,Yp)==0
